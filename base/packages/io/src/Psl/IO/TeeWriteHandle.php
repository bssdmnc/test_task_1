...
final class TeeWriteHandle implements BufferedWriteHandleInterface, CloseHandleInterface
{
    ...
    /**
     * Flush any buffered output to the underlying handles.
     *
     * Drains any bytes that the primary handle accepted but the secondary has
     * not yet accepted. If the pending buffer is non-empty, writes it to the
     * secondary handle in full (blocking until complete).
     *
     * @throws Exception\AlreadyClosedException If the handle has been closed.
